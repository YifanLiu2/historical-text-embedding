...
def sentence_to_embedding(sentence, embeddings):
    ...
    word_embeddings = np.zeros((len(words), embed_dim))
...
def corpus_to_embeddings(corpus, embeddings):
    """Pads embedded sentences to the same length."""
    
    embedding_sentences = [sentence_to_embedding(doc, embeddings) for doc in corpus]
    padded_embeddings = pad_sequence(embedding_sentences, batch_first=True, padding_value=0)
    return padded_embeddings


###### Extract Features for BERT ######
def corpus_to_bert_embeddings(corpus, model, tokenizer):
    # tokenize inputs
    inputs = tokenizer(corpus, padding=True, truncation=True, max_length=512, return_tensors="pt")
    ...
    with torch.no_grad():
        ...
        token_embeddings = hidden_states[-2]  # [batch_size, seq_length, 768]
...
def evaluate(model, dataset, batch_size, device):
    ...
    precision = precision_score(all_true_labels, all_predictions, average=avg_type)
    recall = recall_score(all_true_labels, all_predictions, average=avg_type)
    f1 = f1_score(all_true_labels, all_predictions, average=avg_type)
...
def main_eval_loop(model_path, is_bert, corpus_path, label_dir, output_dir, tokenizer_path=None, input_label=None):
    ...
    if is_bert:  # Load BERT model
        ...
        embeddings = corpus_to_bert_embeddings(corpus, model, tokenizer)
    else:  # Load FastText model
        try:
            model = FastText.load(model_path)
        except Exception as e:
            raise ValueError(f"Failed to load FastText model: {e}")
        embeddings = corpus_to_embeddings(corpus, model)
    ...
    with open(output_path, "a") as f:
        for file_name, label_path in label_paths:
            ...
            dataset = create_dataset(embeddings, labels)

            # 5 fold cross validation
            folds = k_fold_split(dataset, k=5)
            metrics = {'acc': [], 'prec': [], 'recal': [], 'f1': []}

            for i, (train_dataset, val_dataset) in enumerate(folds):
                print(f"evaluate metadata: {file_name}, fold {i + 1}/5")
                model = SequenceClassifier(embed_dim, 256, num_classes).to(device)
                train(model, train_dataset, 8, 100, 0.001, device)
                acc, prec, recal, f1 = evaluate(model, val_dataset, 8, device)
                
                metrics['acc'].append(acc)
                metrics['prec'].append(prec)
                metrics['recal'].append(recal)
